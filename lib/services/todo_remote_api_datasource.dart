...
class ToDoRemoteAPIDatasource implements TodoDatasource {
  final String apiURL = "http://todoapi-env.eba-3my22sx6.ap-southeast-2.elasticbeanstalk.com/";
  ...
  Future<int> addTodo(ToDo t) async {
    http.Response response = await http.post(Uri.parse(apiURL), body: t.toMap());
    if (response.statusCode != 200) {
      return 0;
    } else {
      return 1;
  ...
  Future<int> updateTodo(ToDo t) async {
    http.Response response = await http.patch(Uri.parse(apiURL + t.id!), body: t.toMap());
    if (response.statusCode != 200) {
      return 0;
    } else {
      return 1;
